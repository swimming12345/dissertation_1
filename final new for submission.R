...
cost_parms <-c(
  ...
  cost1 = 4470,            # Chronic HCV infection (per year) cost 
  cost2 = 4380,            # Compensated cirrhosis (per year) cost
  cost3 = 6060,            # Decompensated cirrhosis (per year) cost 
  cost4 = 9900             # Decompensated cirrhosis (per year) cost 
...
plot(0, xlim=c(-1100000, 10000000), ylim=c(axis_y1, 200000), xlab="Incremental DALYs",ylab="Incremental Costs THB (Million)", pch=18, col=colours,
...
#plot(0, xlim=c(-100000, 300000), ylim=c(axis_y1, 100000), xlab="Incremental DALYs",ylab="Incremental Costs THB (Million)", pch=18, col=colours,
#     main="Incremental cost-effectiveness ratio (ICER) plane",yaxt="n",xaxt="n" )
...
base_Cost_Qal <- icer_calculate2(Treatment_Cost = 24000,
                                 Screening_Cost = 1800,
                                 Confirming_Cost = 1500,
                                 Extra_Cost = 4000,
                                 Fibrosis_Rate = 0.73,
...
#                   ICER_r1_Inc_Cost ICER_r1_Qal_gain
# total_base_dis        -80320.82         161844.7
base_Cost_Qal
base_ICER <- base_Cost_Qal[,1]/base_Cost_Qal[,2]
base_QalPerCost <- base_Cost_Qal[,2]/base_Cost_Qal[,1]

#function find Upper&Lower bound ICER
find_ULbound_ICER <- function(Treatment_Cost=c(24000,24000),
                              ...
                              Sens=0.985){
  bound <- icer_calculate2(Treatment_Cost = Treatment_Cost,
                           Screening_Cost = Screening_Cost,
                           Confirming_Cost = Confirming_Cost,
                           Extra_Cost = Extra_Cost,
                           Fibrosis_Rate = Fibrosis_Rate,
  ...
  bound <- bound%>% mutate(UL_Difference = abs(V1 - V2))
  colnames(bound) <- c("Lower_Bound", "Upper_Bound", "UL_Difference")
  return(bound)
  
}
# Doing for all eight parameters with the outcome of Incremental cost effective ratio 

bound_Treatment_Cost <- find_ULbound_ICER(Treatment_Cost= c(min(Treatment_Cost),max(Treatment_Cost)))

bound_Screening_Cost <- find_ULbound_ICER(Screening_Cost= c(min(Screening_Cost),max(Screening_Cost)))

bound_Confirming_Cost <- find_ULbound_ICER(Confirming_Cost= c(min(Confirming_Cost),max(Confirming_Cost)))

bound_Extra_Cost <- find_ULbound_ICER(Extra_Cost= c(min(Extra_Cost),max(Extra_Cost)))

bound_Fibrosis_Rate <- find_ULbound_ICER(Fibrosis_Rate = c(min(Fibrosis_Rate ),max(Fibrosis_Rate )))

bound_Compensate_Rate <- find_ULbound_ICER(Compensate_Rate= c(min(Compensate_Rate),max(Compensate_Rate)))

bound_Decompensate_Rate <- find_ULbound_ICER(Decompensate_Rate= c(min(Decompensate_Rate),max(Decompensate_Rate)))

bound_Total_HCC_Rate <- find_ULbound_ICER(Total_HCC_Rate= c(min(Total_HCC_Rate),max(Total_HCC_Rate)))
...
####checking
icer_calculate2(Treatment_Cost=c(24000,24000),
                Screening_Cost=c(1800,1800),
                Confirming_Cost=c(1500,1500),
                Extra_Cost=c(4000,4000),
                ...
                Sens=0.985)
# ICER_r1_Inc_Cost ICER_r1_Qal_gain
# total_new_dis1        -80320.82          3186799
# total_new_dis2        -80320.82          -957008
...
icer_calculate2(Treatment_Cost=c(24000,24000),
                ...
                Extra_Cost=c(4000,4000),
                Fibrosis_Rate = c(0.73,0.73),
                Compensate_Rate=c(min(Decompensate_Rate ),max(Compensate_Rate )),
                Decompensate_Rate=c(0.53,0.53),
                Total_HCC_Rate=c(0.53,0.53),
                Sens=0.985)
# ICER_r1_Inc_Cost ICER_r1_Qal_gain
# total_new_dis1        -80320.82        1799835.6
# total_new_dis2        -80320.82        -540142.2
...
icer_calculate2(Treatment_Cost=c(24000,24000),
                ...
                Sens=0.985)
# ICER_r1_Inc_Cost ICER_r1_Qal_gain
# total_new_dis1        -80320.82         218829.6
# total_new_dis2        -80320.82         111311.0
...
icer_calculate2(Treatment_Cost=c(24000,24000),
                ...
                Sens=0.985)
# ICER_r1_Inc_Cost ICER_r1_Qal_gain
# total_new_dis1        -35680.69         161844.7
# total_new_dis2       -374249.65         161844.7



####### Qal/cost Tornado plot including the other cost associated with HCV 

find_ULbound_QalPerCost <- function(Treatment_Cost=c(24000,24000),
                                    Screening_Cost=c(1800,1800),
                                    Confirming_Cost=c(1500,1500),
                                    Extra_Cost=c(4000,4000),
                                    Fibrosis_Rate=c(0.73,0.73),
  ...
  bound <- icer_calculate2(Treatment_Cost = Treatment_Cost,
                           ...
                           Sens = Sens)
  bound <- transpose(bound%>%
                       transmute(ICER_Treatment = ICER_r1_Qal_gain/ICER_r1_Inc_Cost))
  bound <- bound%>% mutate(UL_Difference = abs(V1 - V2))
  colnames(bound) <- c("Lower_Bound", "Upper_Bound", "UL_Difference")
  return(bound)
...
bound_Treatment_Cost_Qal <- find_ULbound_QalPerCost(Treatment_Cost= c(min(Treatment_Cost),max(Treatment_Cost)))

bound_Screening_Cost_Qal <- find_ULbound_QalPerCost(Screening_Cost= c(min(Screening_Cost),max(Screening_Cost)))

bound_Confirming_Cost_Qal <- find_ULbound_QalPerCost(Confirming_Cost= c(min(Confirming_Cost),max(Confirming_Cost)))

bound_Extra_Cost_Qal <- find_ULbound_QalPerCost(Extra_Cost= c(min(Extra_Cost),max(Extra_Cost)))

bound_Fibrosis_Rate_Qal <- find_ULbound_QalPerCost(Fibrosis_Rate = c(min(Fibrosis_Rate ),max(Fibrosis_Rate )))

bound_Compensate_Rate_Qal <- find_ULbound_QalPerCost(Compensate_Rate= c(min(Compensate_Rate),max(Compensate_Rate)))

bound_Decompensate_Rate_Qal <- find_ULbound_QalPerCost(Decompensate_Rate= c(min(Decompensate_Rate),max(Decompensate_Rate)))

bound_Total_HCC_Rate_Qal <- find_ULbound_QalPerCost(Total_HCC_Rate= c(min(Total_HCC_Rate),max(Total_HCC_Rate)))

df_all_Qal <- data.frame(rbind(bound_Treatment_Cost_Qal,
                               bound_Screening_Cost_Qal,
                               bound_Confirming_Cost_Qal,
                               bound_Extra_Cost_Qal,
                               bound_Fibrosis_Rate_Qal,
                               bound_Compensate_Rate_Qal,
                               bound_Decompensate_Rate_Qal,
                               bound_Total_HCC_Rate_Qal))

df_all_Qal <- as_tibble(cbind(Parameter=c("Treatment Cost",
                                          "Screening Cost",
                                          "Confirming Cost",
                                          "Extra Cost",
                                          "Fibrosis Rate",
                                          "Compensate Rate",
                                          "Decompensate Rate",
                                          "Total HCC Rate"),df_all_Qal))


order.parameters <- df_all_Qal %>% arrange(UL_Difference) %>%
  mutate(Parameter=factor(x=Parameter, levels=Parameter)) %>%
  select(Parameter) %>% unlist() %>% levels()

width <- 0.95

df_all_Qal_2 <-df_all_Qal %>% 
  ...
  mutate(Parameter=factor(Parameter, levels=order.parameters),
         ymin=pmin(output.value, base_QalPerCost),
         ymax=pmax(output.value, base_QalPerCost),
         xmin=as.numeric(Parameter)-width/2,
         xmax=as.numeric(Parameter)+width/2)
...
ggplot() + 
  geom_rect(data = df_all_Qal_2, 
            aes(ymax=ymax, ymin=ymin, xmax=xmax, xmin=xmin, fill=type,alpha=type)) +
  theme_bw() + 
  theme(axis.title.y=element_blank(), legend.position = 'bottom',
        legend.title = element_blank()) + 
  geom_hline(yintercept = base_QalPerCost) +
  scale_x_continuous(breaks = c(1:length(order.parameters)), 
                     labels = order.parameters)+
  scale_alpha_manual( values = c(0.7, 0.7))+
  coord_flip()
####### final plot with all cost incorporated ########
