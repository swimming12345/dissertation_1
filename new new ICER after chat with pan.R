...
cost_parms <-c(
  ...
  Extra_Cost = 1500+2500   # cost of HCV genotype testing is not taken into out but keeping fibroscan and safety lab after speaking to health care professional
)                          # all costs in TBH
cost_parms
...
cost_utility_list <-function(Cost = c(Treatment_Cost,      # Cost values that are used in overall calculation 
  ...
  utility_new <- fibrosis[21:62]*Fibrosis_Rate+compensate[21:62]*Compensate_Rate+
    decompensate[21:62]*Decompensate_Rate+total_HCC[21:62]*Total_HCC_Rate+
    diff(dthC14)[20:61]*27+diff(dthHCC)[20:61]*17
...
text(ICER_r1[1,2], ICER_r1[1,1]-15000,paste(round(ICER_r1[1,2]),",", round(ICER_r1[1,1])),col = 'brown4',cex = 1.5)
...
plot(0, xlim=c(-100000, 200000), ylim=c(axis_y1, 150000), xlab="Incremental QALYs",ylab="Incremental Costs THB (Million)", pch=18, col=colours,
     main="Incremental cost-effectiveness ratio (ICER) plane",yaxt="n",xaxt="n" )
...
axis(side = 2,at=c(axis_y1,axis_y2,0,50000,100000),labels = c(as.character(axis_y1),as.character(axis_y2),"0","50,000","100,000"))
axis(side = 1,at=c(-100000,0,100000,200000),labels = c("-100000","0","100000","200000"))
...
#points(120825, 11, pch=20,col="brown", cex=2)           # adds current screening method and do nothing scenario
points(153333,124965, pch=20,col="cyan1", cex=2)        # adds scenario 1 point to the ICER
points(153333,80289, pch=20,col="purple", cex=2)        # adds scenario 2 point to the ICER
points(159183,74131, pch=20,col="deeppink", cex=2)      # adds scenario 3 point to the ICER
points(162464,67966, pch=20,col="chartreuse", cex=2)    # adds scenario 4 point to the ICER
...
plot(0, xlim=c(-100000, 800000), ylim=c(axis_y1, 100000), xlab="Incremental QALYs",ylab="Incremental Costs THB (Million)", pch=18, col=colours,
     main="Incremental cost-effectiveness ratio (ICER) plane",yaxt="n",xaxt="n" )
...
plot(0, xlim=c(-100, 1500000), ylim=c(axis_y1, 20000), xlab="Incremental QALYs",ylab="Incremental Costs THB (Million)", pch=18, col=colours,
     main="Incremental cost-effectiveness ratio (ICER) plane",yaxt="n",xaxt="n" )
